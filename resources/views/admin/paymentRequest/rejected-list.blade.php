...
<div class="main-right-wrapper">
    <div class="dashboard-container">
        ...
        <ul class="nav nav-tabs">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="/admin/payment-requests/rejected">Rejected</a>
            </li>
        </ul>
        <br/><br/>

        @if ($paymentRequestsData->count() > 0)
        <h1 class="section-title">Rejected Payment Requests (<span>{{$paymentRequestsData->total()}}</span>)</h1>
        @else
        <!-- For no Pending Payment Request found -->
        <div class="no-users">No Rejected Payment Requests Yet</div>
        @endif
    </div>
    @if ($paymentRequestsData->count() > 0)
