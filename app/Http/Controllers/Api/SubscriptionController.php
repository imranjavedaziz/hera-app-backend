...
class SubscriptionController extends Controller
{
    ...
    /**
     * @OA\Post(
     *      path="/v1/create-subscription-payment-page",
     *      operationId="create-subscription-page",
     *      tags={"Subscription"},
     *      summary="Create payment page for subscription plan",
     *      description="Create payment page for subscription plan",
     *      @OA\RequestBody(
     *        required = true,
     *        description = "Generate Payment page for selected subscription plan",
     *        @OA\JsonContent(
     *             type="object",
     *             @OA\Property(
     *                property="device_type",
     ...
     *  )
     */
    public function createSubscriptionPaymentPage(SubscriptionRequest $request)
    {
        try {
            Log::info("create subscription payment page api calling");
            $requestForm = $request->all();
            if ($requestForm[DEVICE_TYPE] == IOS) {
                $plan = SubscriptionPlan::where(ANDROID_PRODUCT, $requestForm[PRODUCT_ID])->first();
            } else if ($requestForm[DEVICE_TYPE] == ANDROID) {
                $plan = SubscriptionPlan::where(ANDROID_PRODUCT, $requestForm[PRODUCT_ID])->first();
            }
            $ips = Str::uuid()->toString();

            DB::beginTransaction();
            SubscriptionPaymentSetup::create([
                USER_ID => AuthHelper::authenticatedUser()->id,
                IPS => $ips,
                SUBSCRIPTION_PLAN_ID => $plan->id,
                DEVICE_TYPE => $requestForm[DEVICE_TYPE]
            ]);

            $paymentUrl = '/api/v1/make-payment?ips=' . $ips;
            DB::commit();
