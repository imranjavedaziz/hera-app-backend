...
class UserController extends AdminController
{
    ...
    {
        $users = User::select('users.id','users.username','users.first_name', 'users.last_name','users.email','users.role_id','users.country_code','users.phone_no','users.profile_pic','users.status_id','users.deactivated_by','users.deleted_at')
        ->where('deleted_at', NULL)
        ->where('users.role_id','!=',ONE)->where('users.email', '!=', '')->paginate(10);
    ...
    {
    	$user = User::select(ID, USERNAME, ROLE_ID, FIRST_NAME, MIDDLE_NAME, LAST_NAME, PROFILE_PIC, DOB, SUBSCRIPTION_STATUS, CREATED_AT, PHONE_NO, EMAIL, STATUS_ID, DELETED_AT)
